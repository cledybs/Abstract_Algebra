...
class rsa
{
  public:
    ...
    int resto_chino(int);
    string estandar_str(int);
    string convertir_txt(string);
    int cifrar(int);
    int descifrar(int);
...
string rsa::estandar_str(int n)
{
  string estandar_elem = to_string(alfabeto.size());

  string elem = to_string(n);
  while(elem.size() != estandar_elem.size())
    elem = "0" + elem;

  return elem;
}
string rsa::convertir_txt(string M)
{
  //cout << M << endl;
  string P;
...
int main()
{
  ...
  //string P = Cledy.convertir_txt(M);
  int P = 15;
