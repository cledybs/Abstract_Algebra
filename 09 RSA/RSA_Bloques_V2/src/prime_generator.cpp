...
void prime_generator::fillWithMemoryInfo() {
...
vector<int> prime_generator::generateSeed(){
...
int prime_generator::modint(int a,int n){
    int q=a/n;
    if(q<0) q--;
    return a-(n*q);
}
vector<bool> prime_generator::initialize(vector<int> semilla){
    vector<int> Or;
...
void prime_generator::left_Rotate(vector<bool> &vec, int times){
    vec.insert(vec.end(),vec.begin(),vec.begin()+times);
    vec.erase(vec.begin(),vec.begin()+times);
}

vector<bool> prime_generator::DES(int bits){
    vector<bool> K;//para guardar todas las k
    {
        ...
        for(int i=0;i<nBits;i++){
            left_Rotate(c, rotaciones[i]);
            left_Rotate(d, rotaciones[i]);
...
ZZ prime_generator::conversion(vector<bool> vec){
    ZZ salida(0);
    ZZ i(1);
...
ZZ prime_generator::generate_random(int bits){
    return conversion(DES(bits));
}

bool MIller_Rabin_Odd(ZZ n){///Test de primalidad
    ///Prueba los primeros valores de la criba ya que la exponenciacion modular es mas costosas a un modulo
    if(modulo(n, ZZ(2)) == ZZ(0)) return false;
    if(modulo(n, ZZ(3)) == ZZ(0)) return false;
    if(modulo(n, ZZ(5)) == ZZ(0)) return false;
    if(modulo(n, ZZ(7)) == ZZ(0)) return false;
    if(modulo(n, ZZ(11)) == ZZ(0)) return false;
    if(modulo(n, ZZ(13)) == ZZ(0)) return false;
    if(modulo(n, ZZ(17)) == ZZ(0)) return false;
    if(modulo(n, ZZ(19)) == ZZ(0)) return false;
    if(modulo(n, ZZ(23)) == ZZ(0)) return false;
    ...
    for(int i=0;i<9;i++){//Realiza la prueba 9 veces con los primeros primos
        ZZ t(Power_mod(ZZ(firstPrimes[i]), m, n));
        if(t==ZZ(1)||t==ZZ(n-1))
            continue;//probablemente primo
        long j=1;
        while(j<k){
            t= Power_mod(t, ZZ(2), n);
